...
def render_kalite_yonetim_module():
    ...
    kalite_tab = st.selectbox(
        ...
        [
            ...
            "📜 Sözleşme ve Sipariş Formları (FR.71.02.15)",
            "📝 Saha Kayıt ve Risk Analiz Formları",
        ],
    )

    # -----------------------------------------------------------------
    # 1. SEKME: TEKLİF FORMU (FR.71.01.01)
    # -----------------------------------------------------------------
    if kalite_tab == "📄 Teklif Formları (FR.71.01.01)":
        ...
        with st.form("talep_degerlendirme_form"):
            ...
            st.markdown("---")
            col_gorus, col_personel = st.columns(2)
            with col_gorus:
                st.markdown("**LABORATUVAR MÜDÜRÜ GÖRÜŞÜ**")
                st.text_area(
                    "Görüş",
                    value="Proje yapılabilir onayı verilmiştir.",
                    label_visibility="collapsed",
                )
            with col_personel:
                st.markdown("**TALEP HAZIRLAYAN PERSONEL**")
                st.text_input(
                    "Personel", value="Gizem Demir", label_visibility="collapsed"
                )

            if st.form_submit_button("💾 Talep Değerlendirme Formunu Kaydet"):
                st.success(
                    f"Sıra No ({sira_no}) ile teklif formu başarıyla kayıt altına alındı!"
                )

    # -----------------------------------------------------------------
    # 2. SEKME: SÖZLEŞME VE SİPARİŞ FORMU (FR.71.02.15)
    # -----------------------------------------------------------------
    elif kalite_tab == "📜 Sözleşme ve Sipariş Formları (FR.71.02.15)":
        st.markdown(
        ...
        with st.form("sozlesme_form"):
            ...
            with col1:
                ...
                adres_szl = st.text_area("Firma Adresi", value=tutanak_data.get("adres", ""))
            with col2:
                vergi_no = st.text_input("Vergi No / Dairesi", "-- / --")
                telefon_szl = st.text_input(
                    "Telefon Numarası", value=tutanak_data.get("telefon", "")
                )
                belge_no = st.text_input("TÜRKAK / İSGÜM Belge No", "AB-1358-T / 328")

            st.markdown("#### A- Ölçüm Metotları ve Ücretlendirme")
            hizmet_secim = st.selectbox(
                "İstenilen Hizmet",
                [
                    "KATI NUMUNE ASBEST TÜR TAYİNİ",
                    "ASBEST HAVA NUMUNE ANALİZİ",
                    "İŞ HİJYENİ ÖLÇÜMÜ",
                ],
            )
            ...
            if st.form_submit_button("✍️ Sözleşmeyi Onayla ve Kaydet"):
                st.success("Sipariş formu onaylandı ve sözleşme yürürlüğe girmiştir!")

    # -----------------------------------------------------------------
    # 3. SEKME: SAHA KAYIT VE RİSK ANALİZ FORMLARI (İmza ve Şablon Seçenekli)
    # -----------------------------------------------------------------
    elif kalite_tab == "📝 Saha Kayıt ve Risk Analiz Formları":
        st.subheader("📝 Saha Kayıt ve Risk Analiz Formları")
        st.caption(
            "Ayrı ayrı indirilebilir KKD ve Risk Analiz formları (İmza ve personel seçmeli)."
        )

        # Ortak İmza ve Personel Tercih Paneli
        st.markdown("---")
        ...
        # Nihai güncellenmiş imza sözlüğü
        imza_map = {
        ...
        with col_imza_2:
            personel_secimi = st.selectbox(
                "Formu İmzalayacak Personel / Numune Alıcı:", list(imza_map.keys())
            )
        ...
        # --------------------------------------------------
        # SOL KOLON: KKD VE EKİPMAN FORMU İNDİRME
        # --------------------------------------------------
        with col_kkd:
            st.markdown("### 🥽 1. KKD ve Ekipman Formu")
            st.info("Saha hazırlığı, kişisel koruyucu donanımlar ve cihaz listesi.")
                ...
                if os.path.exists(tpl_path):
                    ...
                    if imza_tercihi == "İmzalı Al":
                        ...
                        if imza_path and os.path.exists(imza_path):
                            context["imza"] = InlineImage(doc, imza_path, width=Cm(3.5))
                        else:
                            context["imza"] = ""
                    else:
                        context["imza"] = ""
                    ...
                    doc.render(context)
                    output_filename = f"KKD_Formu_{tutanak_data.get('teklif_no')}.docx"
                    ...
                    st.error("⚠️ 'templates/kalite_saha_kayit_kkd.docx' şablonu bulunamadı.")

        # --------------------------------------------------
        # SAĞ KOLON: RİSK ANALİZİ FORMU İNDİRME
        # --------------------------------------------------
        with col_risk:
            ...
            if st.button("📄 Risk Analizi Formunu Oluştur ve İndir", key="btn_risk"):
                if risk_turu == "Asbestli Malzeme Risk Analizi":
                    tpl_path = "templates/kalite_saha_kayit_risk_asbestli.docx"
                else:
                    tpl_path = "templates/kalite_saha_kayit_risk.docx"
                ...
                if os.path.exists(tpl_path):
                    ...
                    doc.render(context)
                    output_filename = f"Risk_Analizi_{tutanak_data.get('teklif_no')}.docx"
                    doc.save(output_filename)
