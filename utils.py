...
def read_tutanak_details(tutanak_path):
    ...
    return {
        "musteri_adi": musteri_adi, "MUSTERI_ADI": musteri_adi,
        "firma_adi": musteri_adi, "FIRMA_ADI": musteri_adi,
        "adres": adres, "ADRES": adres,
        "santiye_adresi": adres, "SANTIYE_ADRESI": adres,
        "pafta": pafta, "ada": ada, "parsel": parsel,
        "pafta_ada_parsel": pafta_ada_parsel, "PAFTA_ADA_PARSEL": pafta_ada_parsel
    }
...
def parse_asbest_tutanak(file):
    ...
    for idx in range(min(10, len(df_raw))):
        ...
        if "Telefon Numarası:" in row_text:
            ...
            if m and m.group(1).strip(): info["telefon"] = m.group(1).strip()
